...
:: Compilation du projet test (controller, etc.)
set TEST_SRC_DIR=test\java
...
javac -cp "%JAR_FILE%;%TOMCAT_HOME%\lib\servlet-api.jar" -d "%TEST_BUILD_DIR%" %TEST_FILES%

:: Copie des classes du test dans WEB-INF/classes
if not exist "test\WEB-INF\classes" mkdir "test\WEB-INF\classes"
xcopy "%TEST_BUILD_DIR%" "test\WEB-INF\classes" /E /I /Y
